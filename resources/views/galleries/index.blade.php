...
@section('content')
    <div class="container">
        ...
        <div class="text-3xl font-bold text-gray-900 mb-4">{{ __('Gallery') }}</div>

        <div class="mb-4">
            <a href="{{ route('galleries.create') }}"
                class="bg-primary text-white px-4 py-2 rounded hover:bg-primary/90 transition duration-200">{{ __('Tambah Foto') }}</a>
        </div>
            ...
            <tbody>
                @foreach ($galleries as $gallery)
                    <tr class="bg-white border-b">
                        <td scope="row" class="px-6 py-4"> {{ $gallery->name }}</td>
                        <td class="px-6 py-4">
                            <img loading="lazy" src="{{ asset('storage/' . $gallery->path) }}" alt="{{ $gallery->name }}"
